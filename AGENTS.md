...
- **Driving dimensions** (length, diameter, radius, angle) are promises like constraints: they keep their typed value through drag, save/reopen, and show on the sketch; conflicts refuse and leave the sketch unchanged.
- **Arcs** are first-class sketch entities (with lines/circles); closed profiles may mix lines and arcs; tangent joins to lines should hold through drag.
